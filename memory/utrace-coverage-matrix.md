...
| Unmodeled event families | partial | Any declared event not in `EVENT_COVERAGE` | `unmodeled.events` | Dashboard surfaces raw/unimplemented trace families with observed counts and one generic decoded payload sample per event type (e.g. `Object.*`, `NetTrace.*`) before dedicated provider semantics exist. |
