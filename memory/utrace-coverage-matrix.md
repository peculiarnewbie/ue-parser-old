...
| Unmodeled event families | partial | Any declared event not in `EVENT_COVERAGE` | `unmodeled.events` | Dashboard now surfaces raw/unimplemented trace families with observed counts and one generic decoded payload sample per event type. This gives immediate visibility for families such as `Object.*`, `Callstack.*`, `NetTrace.*`, `TaskTrace.*`, `File.*`, and `IoStore.*` before dedicated provider semantics exist. |
...
| CPU profiler | partial | `CpuProfiler.EventSpec`, `CpuProfiler.MetadataSpec`, `CpuProfiler.Metadata`, `CpuProfiler.EventBatchV3`, `CpuProfiler.EndThread` | `cpu.specs`, `cpu.metadata`, `cpu.scopes`, `cpu.threads`, `cpu.end_threads` | Plain scope specs, metadata specs/records, bounded CBOR metadata values, rendered metadata names, V3 scope batches, and thread-end markers are decoded and aggregated. `EventBatchV3` keeps timer stacks, coroutine stacks, reconstructed cycle state, and initial late-connect base cycles from enclosing known trace scope events across batch flushes per thread. Metadata-backed scopes are summarized by metadata spec and rendered name when resolvable, with bounded interval samples marked by attribution source (`inline` or `restored_stack`). Coroutine suspend/resume records restore saved scope stacks and preserve active-time duration across switches. Full emitted CPU timelines remain incomplete. |
| Frames | partial | `Misc.BeginFrame`, `Misc.EndFrame`, CPU metadata frame scopes, GPU frame boundaries | `frames`, `frame_correlation`, `gpu.frames` | Raw frame markers are decoded. Bounded frame correlation joins CPU metadata `Frame N` scopes with queue-local GPU frame buckets by frame number. Full frame windows and CPU/GPU timestamp calibration are not emitted yet. |
...
| Loading/assets | partial | `LoadTime.ClassInfo` | `loading.classes` | Load-time class declarations are decoded into a class pointer/name catalog. Asset, object, package, and full load-time analyzer events are not decoded yet. |
...
CPU/GPU calibration, and full unbounded timelines remain the biggest semantic
gaps.
