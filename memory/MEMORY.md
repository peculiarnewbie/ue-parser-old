...
- [UAsset coverage tracking](uasset-coverage-tracking.md) — per-class decode scoreboard in `docs/asset-coverage.md`; measure against MBPrototype (not engine tree); ~18% decode today; priority: raw-tail fallback → AnimSequence/Montage → Texture2D → StaticMesh → SkeletalMesh
- [UTrace coverage matrix](utrace-coverage-matrix.md) — agent-facing parser coverage status; dashboard currently emits CPU/thread/frame basics, while counters/GPU/memory/loading/logs/callstacks remain explicit gaps
